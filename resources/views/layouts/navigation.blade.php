...
<div x-data="{ open: false }" class="contents lg:block">
    ...
    <nav
                    ...
                    <div x-data="{ expanded: {{ request()->routeIs($link['active']) ? 'true' : 'false' }} }">
                        ...
                        <div x-show="expanded" x-transition:enter="transition ease-out duration-200"
                            ...
                            @foreach ($link['submenu'] as $sublink)
                                <x-nav-link :href="route($sublink['route'])" :active="request()->routeIs($sublink['active'])"
        ...
        <div
            ...
            <x-dropdown align="left" width="48" position="top">
                <x-slot name="trigger">
                    <button
                        class="flex items-center w-full px-3 py-2 text-sm font-medium
    ...
    <div class="lg:hidden">
        ...
        <div x-show="open" x-transition:enter="transform transition-transform duration-300"
            ...
            class="fixed top-0 left-0 bottom-0 w-64 bg-white dark:bg-gray-800 z-50 flex flex-col shadow-xl" x-cloak>
                ...
                @foreach ($navLinks as $link)
                    @if (isset($link['submenu']))
                        ...
                        <div x-data="{ expanded: {{ request()->routeIs($link['active']) ? 'true' : 'false' }} }">
                            ...
                            {{-- SUBMENÚ MÓVIL --}}
                            <div x-show="expanded" x-transition class="mt-2 ml-4 space-y-1">
                                @foreach ($link['submenu'] as $sublink)
                                    <x-responsive-nav-link :href="route($sublink['route'])" :active="request()->routeIs($sublink['active'])" class="text-sm">
                                        {{ __($sublink['name']) }}
